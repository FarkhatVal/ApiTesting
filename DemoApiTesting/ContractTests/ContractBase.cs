...
namespace DemoApiTesting.ContractTests
{
    public abstract class ContractBase
    {
        ...
        protected async Task CheckValidationResponseMessageBySchemaIncorrectPageAsync(HttpResponseMessage response,
            JSchema schema)
        {
            JObject jObject = JObject.Parse(await response.Content.ReadAsStringAsync());
            bool result = jObject.IsValid(schema, out IList<string> msg);
            Assert.IsTrue(result, "Ождалось отсутствие страницы, а получили строки" + string.Join("\n", msg.ToArray()));
        }

        protected async Task CheckValidationResponseMessageBySchemaIncorrectPageByStatusCodeAsync(
            HttpResponseMessage response, JSchema schema)
        {
            Assert.IsTrue(response.StatusCode == HttpStatusCode.NotFound,
                "Ождалось отсутствие страницы и ошибку NotFound, а получили: " + response);
        }
    }
}